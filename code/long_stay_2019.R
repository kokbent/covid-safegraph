...
drive_download("sdm-consolidated-2019.csv", "data/sdm-consolidated.csv",
               overwrite = T)
...
home_time <- cbind(dat[,.(date, origin_census_block_group)], dat[,`0-25`:`26-50`])
df <- home_time %>%
  pivot_longer(cols = `0-25`:`26-50`,
...
p1.1 <- df2 %>%
  filter(week %in% 2:39) %>%
  ggplot() +
  geom_line(aes(x=week_date, y=1-long_stay, colour=age_grp), lwd = 1.1, show.legend = F) +
  scale_colour_manual(values = c("#018571", "#80cdc1", "#dfc27d", "#a6611a"),
                      name = "Age group") +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme_minimal() +
  labs(title = "2019 Mobility trend of Florida (fraction staying at home <= 75%)",
       x = "", y = "") +
...
p3 <- df4 %>%
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme_minimal() +
  labs(title = "2019 Mobility trend of Dade, Broward and Palm Beach (fraction staying at home <= 75%)",
       x = "", y = "") +
  theme(legend.position = c(0.85, 0.85))
...
p4 <- df5 %>%
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme_minimal() +
  labs(title = "2019 Relative activity among age groups of Dade, Broward and Palm Beach",
       x = "", y = "") +
  theme(legend.position = c(0.85, 0.85))
